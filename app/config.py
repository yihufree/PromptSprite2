...
# 2026-09-22（用户要求 3）：**版本升为 V2.3.0** —— 本轮为**功能增强**，未改库结构
#   （仍 schema v5）、未改导入导出格式（仍 JSON v6）：
#     · 导出范围确认对话框：「导出当前分类」前弹整库结构树，已选分支及其全部下属加重显示；
#     · 批量删除本分支全部数据（项目类别 / 根目录 / 分类）：多道确认 + 双重备份 +
#       软/硬删除可选 + 共享分类保护 + 条目位置语义保护；
#     · 自动推荐标签不再受"字段隐藏"影响；结构化向导手输名称不再丢失；详情区重复刷新修复；
#     · 「设置 → 关于」内容精简重写（以功能与使用为主）、开源仓库改为可点击链接、
...
REPO_URL = "https://github.com/yihufree/PromptSprite2"
